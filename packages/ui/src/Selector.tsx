					...
					>
						...
						{!items && (
							...
							>
								...
								<input
									ref={inputRef}
									type="text"
									placeholder={t('common.selector.placeholder')}
									value={query}
						...
						>
							{filteredOptions.length === 0 ? (
								<div className="py-3 text-center text-xs text-(--text-muted)">{t('common.selector.description')}</div>
							) : (
								filteredOptions.map((option) => {
									...
									return (
										<button
											...
											className={`flex w-full items-center justify-between rounded-xl text-sm transition-colors ${isSelected ? 'bg-(--active-accent)/20 text-(--accent)' : 'text-(--text) hover:bg-(--border)/5'}`}
											style={{
