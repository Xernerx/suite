...
export function NotFound() {
	...
	return (
		<div
			className="flex min-h-screen w-full flex-col items-center justify-center overflow-hidden font-sans"
			style={{
				padding: 'var(--ui-gap)',
			}}
		>
			...
			>
				...
				>
					...
					<Logo
						...
						className="absolute h-14 w-14 text-(--accent) opacity-40 blur-lg fill-current"
						style={{
							display: 'block',
							width: '56px',
							height: '56px',
				...
				>
					<div
						...
						style={{
							padding: 'calc(var(--ui-gap) * 0.4) var(--ui-gap)',
					...
					>
						...
						<span className="text-[11px] font-bold tracking-widest text-(--text-muted) uppercase">{t('common.notfound.description')}</span>
					</div>

					<h1 className="text-3xl font-extrabold tracking-tight text-(--text) md:text-5xl">{t('common.notfound.title')}</h1>

					<p className="mx-auto max-w-sm text-base leading-relaxed text-(--text-muted)">{t('common.notfound.description2')}</p>
				</motion.div>
