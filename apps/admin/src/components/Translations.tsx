...
export default function Translations() {
	...
	const localesList: Array<{ code: string; label: string; coverage: number }> = Array.isArray(localesData)
		? localesData
		: localesData && typeof localesData === 'object'
			? Object.values(localesData)
			: [];
	...
	const isDirty = useMemo(() => {
		const allKeys = Object.keys({ ...sourceDict, ...targetDict, ...initialTargetDict });
		...
		async function fetchDictionaries() {
			...
			try {
				const [sourceRes, targetRes] = await Promise.all([
					fetch(getEnvUrl(`https://api.xernerx.com/secure/dictionary/en-GB`), { credentials: 'include' }),
					fetch(getEnvUrl(`https://api.xernerx.com/secure/dictionary/${selectedLocale}`), { credentials: 'include' }),
				]);
	...
	const handleInputChange = (key: string, value: string) => {
		setTargetDict((prev) => ({ ...prev, [key]: value }));
	};
						...
						>
							...
							<div className="flex flex-col rounded-2xl border border-(--border)/10 bg-(--background)/50 backdrop-blur-md p-5" style={{ gap: 'calc(var(--ui-gap) * 0.75)' }}>
								...
								<ul className="flex flex-col text-xs text-(--text-muted) list-disc list-outside ml-4" style={{ gap: 'calc(var(--ui-gap) * 0.4)' }}>
									...
									<li>{t('admin.translations.welcome.guide.step2', {}, 'You can choose to either review new entries or modify existing ones')}</li>
									<li>{t('admin.translations.welcome.guide.step3', {}, 'There is no deadline or haste upon these translations, take your time.')}</li>
									<li>
										{t('admin.translations.welcome.guide.step4', {}, 'When you are changing keys the push to review button will become available, only press this once ready.')}
									</li>
									<li>{t('admin.translations.welcome.guide.step5', {}, 'While editing try to not close or refresh the page, it may lose unsaved progression.')}</li>
									<li className="leading-relaxed">
										{t(
											'admin.translations.welcome.guide.step6',
											{},
											'After submitting you will see your translated keys untranslated again, this is normal. The live environment cannot update manually, you have to wait for an admin to merge this updated dictionary with the live environment. You can speed up this progress by asking an admin directly to merge these changes'
										)}
									</li>
								</ul>
							</div>
							...
							<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
								<div className="flex flex-col rounded-2xl border border-(--border)/10 bg-(--background)/50 backdrop-blur-md p-5" style={{ gap: 'calc(var(--ui-gap) * 0.5)' }}>
									...
									>
										<span>{t('admin.translations.welcome.startTranslating', { count: missingKeys.length })}</span>
										<ArrowRight size={14} />
									</button>
								</div>
						...
						<div>
							{missingKeys.length === 0 ? (
								<div
									...
									style={{ padding: 'calc(var(--ui-gap) * 2.5)', gap: 'var(--ui-gap)' }}
								>
									...
									<div className="flex flex-col gap-1">
										...
										<p className="text-sm text-(--text-muted)">{t('admin.translations.new.noMissingKeys', { locale: currentLocaleObj?.label || selectedLocale })}</p>
									</div>
										...
										>
											<div className="flex items-center justify-between">
												...
												<span className="text-xs font-medium text-(--text-muted)">
													{t('admin.translations.new.cardCounter', { current: newIndex + 1, total: missingKeys.length })}
												</span>
											</div>
