...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
	try {
		...
		const VoteModel = db.models.guilds.Vote as any;
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
	try {
		...
		// 2. Record vote in new collection
		...
		const totalVotes = await VoteModel.countDocuments({ guildId: id });
		const StatModel = (db.models.guilds as any).Stat;
		const latestStat = await StatModel.findOne({ id }).sort({ timestamp: -1 }).lean();

		await StatModel.create({
			id,
			members: latestStat?.members ?? 0,
			bots: latestStat?.bots ?? 0,
			messages: latestStat?.messages ?? 0,
			boosts: latestStat?.boosts ?? 0,
		...
		return NextResponse.json({ success: true, message: 'Vote recorded and 100 credits awarded' }, { status: 200 });
	} catch (error) {
