...
export default function Profile() {
	...
	const handleSave = async () => {
		...
		try {
			...
			toast({ title: t('auth.profile.toast.success', {}, 'Profile updated successfully!'), type: 'success' });
		} catch (error) {
			toast({ title: t('auth.profile.toast.error', {}, 'Failed to save changes. Please try again.'), type: 'error' });
		} finally {
			setIsSaving(false);
		}
	};
				...
				<div className='flex flex-col rounded-3xl border border-(--border)/10 bg-(--foreground) shadow-sm' style={{ padding: 'var(--ui-gap)', gap: 'var(--ui-gap)' }}>
					...
					<div className='grid grid-cols-1 md:grid-cols-2' style={{ gap: 'var(--ui-gap)' }}>
						<div className='flex flex-col' style={{ gap: 'calc(var(--ui-gap) * 0.4)' }}>
							...
							<Selector
								...
								options={[
									{ label: t('auth.profile.gender.male', {}, 'Male'), value: 'male' },
									{ label: t('auth.profile.gender.female', {}, 'Female'), value: 'female' },
									{ label: t('auth.profile.gender.other', {}, 'Other'), value: 'other' },
								]}
							/>
						</div>
						<div className='flex flex-col' style={{ gap: 'calc(var(--ui-gap) * 0.4)' }}>
					...
					<div className='flex flex-col' style={{ gap: 'calc(var(--ui-gap) * 0.4)' }}>
						...
						<Selector
							...
							options={[
								{ label: t('auth.profile.privacy.public', {}, 'Public'), value: 'public' },
								{ label: t('auth.profile.privacy.limited', {}, 'Limited'), value: 'limited' },
								{ label: t('auth.profile.privacy.private', {}, 'Private'), value: 'private' },
							]}
						/>
					</div>
