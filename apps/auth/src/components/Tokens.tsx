...
export default function Tokens() {
	...
	const fetchTokens = async () => {
		try {
			...
			toast({ title: t('auth.tokens.toast.loadError', { error: (error as Error).message }), type: 'error' });
		} finally {
			setIsLoading(false);
		}
	};
		...
		<div
			...
			<div
				...
				<div className='flex items-center' style={{ gap: 'calc(var(--ui-gap) * 0.75)' }}>
					<div className='flex items-center justify-center rounded-2xl bg-(--accent)/10 text-(--accent) shrink-0' style={{ width: 'calc(var(--ui-gap) * 2.5)', height: 'calc(var(--ui-gap) * 2.5)' }}>
						<Key size={24} />
					</div>
	...
	useEffect(() => {
		...
		const fetchFullToken = async () => {
			...
			try {
				...
				toast({ title: t('auth.tokens.manage.loadError', { error: (error as Error).message }), type: 'error' });
				onClose();
			} finally {
				setIsLoading(false);
			}
						...
						{isLoading || !token ? (
							...
							<>
								<div className='flex items-center justify-between'>
									...
									<button onClick={handleDelete} className='p-2 text-(--accent-red) hover:bg-(--accent-red)/10 rounded-xl transition-colors' title={t('auth.tokens.manage.revokeTitle')}>
										<Trash2 size={18} />
									</button>
								</div>
								...
								<div className='flex flex-col rounded-2xl bg-(--background) border border-(--border)/10 p-4' style={{ gap: 'calc(var(--ui-gap) * 0.5)' }}>
									...
									<div className='flex items-center gap-2'>
										<div className='flex-1 flex items-center rounded-xl bg-(--foreground) border border-(--border)/20 px-3 py-2 overflow-hidden relative'>
											<code className={`text-sm font-mono truncate transition-all duration-300 ${isTokenVisible ? 'text-(--text)' : 'text-(--text-muted) blur-sm select-none'}`}>{token.id}</code>
										</div>
										<button
