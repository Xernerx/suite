...
import { ArrowLeft, Box, ChevronRight, FileCode2 } from 'lucide-react';
...
export default async function PackageDetails({ params }: { params: Promise<{ package: string[] }> }) {
		...
		return (
			<div className="max-w-4xl mx-auto py-12 px-6 lg:px-8 w-full text-center">
				...
				<p className="text-(--text-muted)">
					Could not load documentation for {pkgName}@{version}.
				</p>
					...
					<div className="flex flex-col gap-1.5 mt-1 border-t border-(--border)/10 pt-2">
						{blockTags.map((tag: any, idx: number) => (
							<div key={idx} className="flex gap-2">
								...
								<span className="text-sm opacity-90 break-words">{tag.content?.map((c: any, i: number) => c.text).join('')}</span>
							</div>
						))}
					</div>
				)}
	...
	const sidebarItems = [
		...classes.map((c: any) => ({ label: c.name, href: `#class-${c.name}`, category: 'Classes' })),
		...interfaces.map((i: any) => ({ label: i.name, href: `#interface-${i.name}`, category: 'Interfaces' })),
		...functions.map((f: any) => ({ label: f.name, href: `#function-${f.name}`, category: 'Functions' })),
	];
	...
	return (
		<div className="max-w-7xl mx-auto py-12 px-6 lg:px-8 w-full selection:bg-(--accent) selection:text-white">
			...
			<div className="space-y-16">
				{classes.length > 0 && (
					<section>
						<h2 className="text-3xl font-bold text-(--text) mb-8">Classes</h2>
									...
									<div key={c.id} id={`class-${c.name}`} className="bg-(--foreground)/30 border border-(--border)/10 rounded-2xl p-6 scroll-mt-24">
										...
										<div className="space-y-6 mt-6">
											{constructors.length > 0 && (
												<div>
													<h4 className="text-sm font-bold text-(--text-muted) uppercase tracking-wider mb-3">Constructor</h4>
											...
											{properties.length > 0 && (
												<div>
													<h4 className="text-sm font-bold text-(--text-muted) uppercase tracking-wider mb-3">Properties</h4>
																...
																{properties.map((prop: any) => (
																	<tr key={prop.id} className="hover:bg-(--foreground)/30 transition-colors">
																		...
																		<td className="px-4 py-4 text-(--text-muted)">
																			{renderComment(prop.comment) || <span className="opacity-50 italic">No description</span>}
																		</td>
																	</tr>
																))}
															</tbody>
											...
											{methods.length > 0 && (
												<div>
													<h4 className="text-sm font-bold text-(--text-muted) uppercase tracking-wider mb-3">Methods</h4>
													<div className="space-y-3">
				...
				{interfaces.length > 0 && (
					<section>
						<h2 className="text-3xl font-bold text-(--text) mb-8">Interfaces</h2>
						<div className="space-y-8">
