...
import { Button, Toggle, Confirm, Modal } from '@xernerx/ui';
	...
	if (loading) {
		return (
			<div className="flex items-center gap-1.5 rounded-xl border border-(--border)/10 bg-(--background)/50 backdrop-blur-md pl-2 pr-2 py-1 text-xs text-(--text) shadow-sm w-max">
				...
				<span className="font-medium truncate max-w-[120px]">{t('account.common.description')}</span>
			</div>
		);
	}
	if (!profile) {
		return (
			<div className="flex items-center gap-1.5 rounded-xl border border-red-500/10 bg-red-500/5 backdrop-blur-md pl-2 pr-2 py-1 text-xs text-red-500 shadow-sm w-max">
				<span className="font-medium truncate max-w-[120px]">{t('account.common.description2')}</span>
			</div>
		);
	}
	...
	return (
		<div className="flex flex-col gap-2 w-full overflow-visible" ref={ref}>
			...
			<div className="relative w-full">
				...
				>
					<span className="text-(--text-muted) font-medium">{placeholder || t('account.tokens.manage.ownersPlaceholder')}</span>
						...
						>
							...
							>
								...
								<input
									ref={inputRef}
									type="text"
									placeholder={t('account.tokens.manage.ownersPlaceholder')}
									value={query}
							...
							>
								{!isValidId ? (
									<div className="py-4 text-center text-xs text-(--text-muted)">{t('account.tokens.manage.invalidIdPrompt')}</div>
								) : loading ? (
									...
									<div className="py-4 text-center text-xs text-(--text-muted)">{t('account.tokens.manage.userAlreadyAdded')}</div>
								) : (
									<div className="py-4 text-center text-xs text-(--text-muted)">{t('account.tokens.manage.userNotFound')}</div>
								)}
							</div>
						</motion.div>
					)}
...
export default function Tokens() {
	...
	const fetchTokens = async () => {
		try {
			...
			toast({
				title: t('account.tokens.toast.loadError', {
		...
		>
			...
			{isLoading ? (
				...
				>
					...
					<p className="text-sm font-medium text-(--text)">{t('account.tokens.empty.title')}</p>
					<p className="text-xs text-(--text-muted) max-w-sm">{t('account.tokens.empty.description')}</p>
				</div>
			) : (
				<div
					...
					<AnimatePresence>
						{tokens.map((token, i) => (
							<motion.div
								...
								onClick={() => setSelectedTokenId(token.id || token._id)}
							...
							>
								<div className="flex items-start justify-between">
									...
									>
										...
										<p className="text-xs font-mono text-(--text-muted)">...{token.id?.slice(-6)}</p>
									</div>
								...
								<div className="flex items-center text-xs font-medium text-(--accent) opacity-0 transition-opacity group-hover:opacity-100 mt-2">
									<Settings2 size={14} className="mr-1.5" /> {t('account.tokens.manageSettings')}
								</div>
							</motion.div>
						))}
					</AnimatePresence>
...
function CreateTokenModal({ isOpen, onClose, onSuccess, userId }: { isOpen: boolean; onClose: () => void; onSuccess: () => void; userId?: string }) {
		...
		try {
			const res = await fetch(`${getEnvUrl('https://api.xernerx.com/')}secure/tokens/${newTokenId}`, {
				...
				body: JSON.stringify({
					...
					status: 'pending',
					owners,
					userId: userId || owners[0],
				}),
			});
			if (!res.ok) throw new Error('Failed to create token');
			toast({
				title: t('account.tokens.create.success'),
				type: 'success',
			});
			onSuccess();
			onClose();
			...
			toast({
				title: t('account.tokens.create.errorTitle'),
				description: t('account.tokens.create.errorDesc', {
					error: (error as Error).message,
				}),
				type: 'error',
			});
	...
	return (
		<Modal
			...
			title={t('account.tokens.create.modalTitle')}
		>
			<form
			...
			>
				...
				>
					<label className="text-sm font-medium text-(--text)">{t('account.tokens.create.nameLabel')}</label>
					<input
						type="text"
						value={name}
						onChange={(e) => setName(e.target.value)}
						placeholder={t('account.tokens.create.namePlaceholder')}
				...
				>
					<label className="text-sm font-medium text-(--text)">{t('account.tokens.manage.ownersLabel')}</label>
					<AsyncUserMultiSelector values={owners} onChange={setOwners} getEnvUrl={getEnvUrl} />
				</div>

				<div className="flex justify-end gap-3 mt-2">
					<Button type="button" variant="outline" onClick={onClose} disabled={isSubmitting}>
						{t('account.tokens.create.cancelButton')}
					</Button>
					<Button type="submit" className="bg-(--accent) text-white hover:bg-(--accent-hover)" disabled={isSubmitting}>
						{isSubmitting ? <Loader2 size={16} className="animate-spin" /> : t('account.tokens.create.submitButton')}
					</Button>
				</div>
			</form>
		</Modal>
...
function ManageTokenModal({ tokenId, onClose, onSuccess }: { tokenId: string | null; onClose: () => void; onSuccess: () => void }) {
	...
	useEffect(() => {
		...
		const fetchFullToken = async () => {
			...
			try {
				...
				toast({
					title: t('account.tokens.manage.loadError', {
						error: (error as Error).message,
					}),
					type: 'error',
				});
	...
	const handleCopy = () => {
		...
		setCopied(true);
		toast({
			title: t('account.tokens.manage.copySuccess'),
			type: 'success',
		});
	...
	const handleUpdate = async (e: React.FormEvent) => {
		...
		try {
			...
			toast({
				title: t('account.tokens.manage.updateSuccess'),
				type: 'success',
			});
			onSuccess();
			onClose();
			...
			toast({
				title: t('account.tokens.manage.updateError'),
				type: 'error',
			});
		} finally {
			setIsSaving(false);
	...
	const handleDelete = async () => {
		...
		try {
			...
			toast({
				title: t('account.tokens.manage.deleteSuccess'),
				type: 'success',
			});
			onSuccess();
			onClose();
			...
			toast({
				title: t('account.tokens.manage.deleteError'),
				type: 'error',
			});
		} finally {
			setIsSaving(false);
	...
	return (
		<>
			<Modal
				...
				title={t('account.tokens.manage.modalTitle')}
			>
				{isLoading || !token ? (
					...
					<>
						<div
							className="flex flex-col rounded-2xl bg-(--background) border border-(--border)/10 p-4"
							style={{
								gap: 'calc(var(--ui-gap) * 0.5)',
							}}
						>
							<label className="text-xs font-semibold uppercase tracking-wider text-(--text-muted)">{t('account.tokens.manage.accessKeyLabel')}</label>
							...
							<p className="text-[11px] text-(--accent-orange) mt-1">{t('account.tokens.manage.securityWarning')}</p>
						</div>
						...
						>
							...
							>
								<label className="text-sm font-medium text-(--text)">{t('account.tokens.manage.nameLabel')}</label>
								<input
									type="text"
									value={name}
									onChange={(e) => setName(e.target.value)}
							...
							{token.botId && (
								...
								>
									<label className="text-sm font-medium text-(--text)">{t('account.common.description3')}</label>
							...
							{token.status !== 'pending' && token.status !== 'suspended' && (
								<div className="flex items-center justify-between rounded-xl border border-(--border)/20 bg-(--background) px-4 py-3">
									...
									>
										<span className="text-sm font-medium text-(--text)">{t('account.tokens.manage.statusTitle')}</span>
										<span className="text-xs text-(--text-muted)">{status === 'active' ? t('account.tokens.manage.statusActive') : t('account.tokens.manage.statusInactive')}</span>
									</div>
							...
							<div className="flex items-center justify-between mt-2">
								<Button type="button" variant="danger" disabled={isSaving} onClick={() => setConfirmDeleteOpen(true)}>
									<Trash2 size={14} className="mr-1.5" /> {t('account.tokens.manage.revokeTitle')}
								</Button>
								<div className="flex justify-end gap-3">
									{isDirty && (
										<Button type="button" variant="outline" onClick={onClose} disabled={isSaving}>
											{t('account.tokens.manage.closeButton')}
										</Button>
									)}
									<Button type="submit" className="bg-(--accent) text-white hover:bg-(--accent-hover) gap-2" disabled={isSaving}>
										...
										{isDirty ? t('account.tokens.manage.saveButton') : t('account.tokens.manage.closeButton')}
									</Button>
								</div>
							</div>
						</form>
			...
			<Confirm
				...
				title={t('account.tokens.manage.revokeTitle')}
				description={t('account.tokens.manage.deleteConfirm')}
				confirmText={t('account.tokens.manage.revokeTitle')}
				cancelText={t('account.tokens.create.cancelButton')}
